...
public class DockTitleBar extends HBox implements EventHandler<MouseEvent> {
	...
	public DockTitleBar(DockNode dockNode) {
		...
		minimizeButton.setOnAction(e -> dockNode.setMinimized(true));
		...
		maximizeButtnon.setOnAction(e -> dockNode.setFloating(false).setMaximized(true));
	 ...
	 *
	 * @return The button used for closing this title bar and its associated
	 *         dock node.
	 */
	...
	/**
	 * The button used for detaching, maximizing, or restoring this title bar
	 * and its associated dock node.
	 *
	 * @return The button used for detaching, maximizing, or restoring this
	 *         title bar and its associated dock node.
	 */
	...
	/**
	 * The button used for minimizing this title bar and its associated dock
	 * node.
	 *
	...
	/**
	 * The mouse location of the original click which we can use to determine
	 * the offset during drag. Title bar dragging is asynchronous so it will not
	 * be negatively impacted by less frequent or lagging mouse events as in the
	 * case of most current JavaFX implementations on Linux.
	 */
	...
	/**
	 * The current node being dragged over for each window so we can keep track
	...
	public void handle(MouseEvent event) {
		if (event.getEventType() == MouseEvent.MOUSE_PRESSED) {
			...
			// it is possible that drag start has not been set if some other
			// node had focus when
			// we started the drag
			if (null == dragStart) {
				dragStart = new Point2D(event.getX(), event.getY());
