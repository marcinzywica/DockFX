...
public class DockEvent extends Event {
	...
	/**
	 * This event occurs when a dock window is being dragged by its title bar
	 * and the mouse enters a node's bounds. Unlike a {@code DragEvent} the dock
	 * over event is handed to all stages that may be interested in receiving
	 * the dock pane.
	 */
	...
	/**
	 * This event occurs when a dock window is being dragged by its title bar
	 * and the mouse is contained in a node's bounds. Unlike a {@code DragEvent}
	 * the dock over event is handed to all stages that may be interested in
	 * receiving the dock pane.
	 */
	public static final EventType<DockEvent> DOCK_OVER = new EventType<DockEvent>(DockEvent.ANY, "DOCK_OVER");

	/**
	 * This event occurs when a dock window is being dragged by its title bar
	 * and the mouse exits a node's bounds. Unlike a {@code DragEvent} the dock
	 * over event is handed to all stages that may be interested in receiving
	 * the dock pane.
	 */
	...
	/**
	 * This event occurs when a dock window is being dragged by its title bar
	 * and the mouse is released over a node's bounds. Unlike a
	 * {@code DragEvent} the dock over event is handed to all stages that may be
	 * interested in receiving the dock pane.
	 */
	...
	/**
	 * Depth z position of the event relative to the origin of the DockEvent's
	 * node.
	 */
	private transient double z;

	/**
	 * Depth position of the event relative to the origin of the DockEvent's
	 * source.
	 *
	 ...
	 * {@code Scene} that contains the DockEvent's source. If the node is not in
	 * a {@code Scene}, then the value is relative to the boundsInParent of the
	 ...
	 * @param x
	 *            The x with respect to the source. Should be in scene
	 *            coordinates if source == null or source is not a Node.
	 * @param y
	 *            The y with respect to the source. Should be in scene
	 *            coordinates if source == null or source is not a Node.
	 * @param screenX
	 ...
	 *            without any further values is constructed based on the scene
	 *            coordinates
	 */
	 ...
	 *            coordinates
	 */
	public DockEvent(Object source, EventTarget target, EventType<? extends DockEvent> eventType, double x, double y,
			double screenX, double screenY, PickResult pickResult) {
	...
	public DockEvent(Object source, EventTarget target, EventType<? extends DockEvent> eventType, double x, double y,
			double screenX, double screenY, PickResult pickResult, Node contents) {
		...
		final Point3D p = com.sun.javafx.scene.input.InputEventUtils.recomputeCoordinates(this.pickResult, null);
		this.x = p.getX();
		this.y = p.getY();
		this.z = p.getZ();
